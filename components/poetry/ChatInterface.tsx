...
export function ChatInterface({ poem }: ChatInterfaceProps) {
  ...
  useEffect(() => {
    const systemMessage: Message = {
      role: "system",
      content: `You are a helpful poetry expert. You're discussing the poem "${
        poem.title.original
      } (${poem.title.translated})" by ${poem.author.name} from the ${
        poem.author.dynasty
      } dynasty. 
          ...
          {messages.slice(1).map((message, index) =>
            message.role === "user" ? (
              <div
                ...
                style={{
                  maxWidth:
                    Math.min(Math.max(message.content.length * 12, 150), 340) +
                    "px",
              ...
              <div
                key={index}
                className="space-y-3 sm:space-y-4 text-xs sm:text-sm mb-6"
              >
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-2 sm:mb-3 w-full">
        {predefinedQuestions.map((q) => (
          <Button
            ...
            variant="outline"
            size="sm"
            className="bg-white hover:bg-gray-50 text-gray-400 border-gray-200 rounded-full px-2 py-1 text-xs shadow h-7"
            onClick={() => handlePredefinedQuestion(q.question)}
            disabled={isLoading}
          >
            <span className="flex items-center gap-1">
              <span className="w-4 h-4 flex items-center justify-center">
                <img src={q.icon} alt={q.label} className="w-4 h-4" />
              </span>
              {q.label}
            </span>
          </Button>
