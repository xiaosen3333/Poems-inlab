...
export function PoetryCanvas({
        ...
        prev.map(el => 
          el.id === elementId 
            ? { 
                ...
                x: Math.max(0, Math.min(canvasRect.width - el.width, startElementX + dx)),
                y: Math.max(0, Math.min(canvasRect.height - el.height, startElementY + dy))
              }
            : el
        )
      )
  ...
  return (
    <div 
      className="relative flex-1 border border-dashed border-gray-200 rounded-2xl bg-white overflow-visible min-h-[500px] sm:min-h-[550px] md:min-h-[600px] h-full" 
      onDragOver={(e) => {
        e.preventDefault()
      ...
      {canvasElements.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center text-gray-400 pointer-events-none">
          <div className="text-center">
            <p className="text-base sm:text-xl text-gray-500">Drag items here to create your visualization</p>
            <p className="text-xs sm:text-sm mt-2 text-gray-400">You can position images on the canvas</p>
          </div>
        </div>
      )}
      ...
      {canvasElements.length > 0 && (
        <div className="absolute inset-0 overflow-visible">
          {canvasElements.map((element) => {
            const visualElement = visualElements.find(ve => ve.id === element.elementId)
            if (!visualElement) {
