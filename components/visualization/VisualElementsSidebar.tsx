...
export function VisualElementsSidebar({
  ...
  useEffect(() => {
    ...
    if (!manualToggleRef.current) {
      ...
      } else if (hasAvailableElements && !sidebarOpen && usedElements.length < visualElements.length) {
  ...
  return (
    <div className="hidden md:block w-0 absolute left-0 top-0 h-full">
      {/* Toggle button */}
      <button 
        className={`absolute ${sidebarOpen ? 'left-[100px]' : 'left-0'} top-1/2 transform -translate-y-1/2 z-30 
        ${sidebarOpen 
          ? 'bg-[#7067DC] text-white' 
          : hasAvailableElements ? 'bg-white text-gray-400 border border-gray-200' : 'bg-white text-gray-300 border border-gray-200 opacity-70'} 
      ...
      <div 
        className={`absolute top-1/2 -translate-y-1/2 left-0 transition-all duration-300 bg-white rounded-2xl p-4 shadow-lg z-20 border border-gray-100 ${
          sidebarOpen && hasAvailableElements ? 'opacity-100 w-[100px] translate-x-0' : 'opacity-0 w-[100px] -translate-x-full pointer-events-none'
        }`}
      >
